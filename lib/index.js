...
var fs = require('fs');
console.log(chalk.blue(figlet.textSync('localiso-cli (Yeah!)', { horizontalLayout: 'full' })));
program
    .version('0.0.1')
    ...
    .parse(process.argv);
var publicApiEndpoint = 'http://{tenant}.localiso.co/pub-api/translations/{projectId}?platformIds[]={platformId}';
...
function dartEnum(response, path) {
...
function dartJson(response, path) {
    response.languages.forEach(function (language) {
        var languageFileContent = '[\n';
        language.translationEntries.forEach(function (translationEntry, key) {

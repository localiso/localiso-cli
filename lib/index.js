...
function javaProperties(response, path) {
    response.languages.forEach(function (language) {
        ...
        fs.writeFile(path + '/messages_' + language.languageCode.replace('-', '_') + '.properties', languageFileContent, function (err) {
            if (err) {
...
function json(response, path) {
    response.languages.forEach(function (language) {
        ...
        languageFileContent += '}';
        fs.writeFile(path + '/' + language.languageCode + '.json', languageFileContent, function (err) {
            if (err) {
                return console.log(err);
            }
        });
